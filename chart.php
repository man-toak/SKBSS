...
<head>
  ...
  <body>
	...
	<?php
		...
		$query = "SELECT class_name , (SELECT COUNT(class_id) FROM student WHERE a.class_id = class_id) AS percentage FROM class a WHERE class_id IN( SELECT class_id FROM student WHERE a.class_id = class_id) ORDER BY percentage DESC";
						  ...
						  function drawChart() {
							var data = google.visualization.arrayToDataTable([
							  ['Class', 'Percentage'],
								...
								{
								echo "['".$row['class_name']."',".$row['percentage']."],";
								}
								?>
								]);

								var options = {
								  title: 'PERCENTAGE OF CLASS AMONG STUDENTS IN SMK BANDAR SEMENYIH ',
								  pieHole: 0.4,
									};
