...
<html>
  ...
  <body>
	...
	?>
	      ...
	      <?php //data state 
            if (isset($_GET['t_ic'])) $t_ic = $_GET['t_ic'];
		else 
			$t_ic = 0;
		...
		include 'dbconnect.php';
           ...
           $query1 = "SELECT t.*, st.* FROM state st, teacher t where t.state_id=st.state_id AND t.t_ic =$t_ic";
		   $result1 = mysqli_query($conn,$query1) or die('SQL error state ');
		   $row1 = mysqli_fetch_array($result1, MYSQLI_ASSOC);
          ?>
		 
		       <?php
                 if (isset($_GET['state_id'])) $state_id = $_GET['state_id'];
		else 
			$state_id = 0;
		...
		include 'dbconnect.php';
           ...
           $query12 = "SELECT * FROM state ";
		   $result12 = mysqli_query($conn,$query12) or die('SQL error state 1');
		...
		include 'dbconnect.php';
		     if (isset($_POST['submit']) && isset($_POST['educationID']))
		     {
			  $educationID = addslashes($_POST['educationID']);
             }
           $query2 = "SELECT t.*, e.* FROM education e, teacher t where t.educationID=e.educationID AND t.t_ic =$t_ic";
		   $result2 = mysqli_query($conn,$query2) or die('SQL error education');
		   $row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC);
          ?>
		 
		       <?php
                if (isset($_GET['educationID'])) $educationID = $_GET['educationID'];
		       else 
			$educationID = 0;
		...
		include 'dbconnect.php';
           ...
           $query11 = "SELECT * FROM education ";
		   $result11 = mysqli_query($conn,$query11) or die('SQL error education 1');
		...
		include 'dbconnect.php';
           ...
           $query3 = "SELECT t.*, g.* FROM gender g, teacher t where t.g_id=g.g_id AND t.t_ic =$t_ic";
		   $result3 = mysqli_query($conn,$query3) or die('SQL error gender');
		   $row3 = mysqli_fetch_array($result3, MYSQLI_ASSOC);
          ?>

	     <?php 
            if (isset($_GET['g_id'])) $g_id = $_GET['g_id'];
		else 
			$g_id = 0;
		...
		$query13 = "SELECT * FROM gender";
		$result13 = mysqli_query($conn,$query13) or die('SQL error gender 1');
		$row13 = mysqli_fetch_array($result13, MYSQLI_ASSOC);
          ?>

         <?php //data class
            if (isset($_GET['t_ic'])) $class_id = $_GET['t_ic'];
		else 
			$t_ic= 0;
		...
		include 'dbconnect.php';
           ...
           $query4 = "SELECT t.*, c.* FROM class c, teacher t where t.class_id=c.class_id AND t.t_ic =$t_ic";
		...
		$query14 = "SELECT *FROM class";
		$result14 = mysqli_query($conn,$query14) or die('SQL error  class');
		$row14 = mysqli_fetch_array($result14, MYSQLI_ASSOC);
          ?>
	
        <?php //data form
            if (isset($_GET['t_ic'])) $form_id = $_GET['t_ic'];
		else 
			$t_ic= 0;
		...
		include 'dbconnect.php';
           ...
           $query5 = "SELECT t.*, f.* FROM form f, teacher t where t.form_id=f.form_id AND t.t_ic =$t_ic";
	...
	<?php  
		...
		if (isset($_POST['add'])) 
			$form_id = addslashes($_POST['form_id']);
		$query15 = "SELECT * FROM form";
		$result15 = mysqli_query($conn,$query15) or die('SQL error form');
      ...
      <div id="heading-breadcrumbs">
        <div class="container">
          <div class="row d-flex align-items-center flex-wrap">
            <div class="col-md-7">
              <h1 class="h2"><?php echo $row['t_name']; ?></h1>
            </div>
            <div class="col-md-5">
              <ul class="breadcrumb d-flex justify-content-end">
                <li class="breadcrumb-item"><a href="homepage_teacher.php">Home</a></li>
                <li class="breadcrumb-item"><a href="info_teacher1.php">Profile</a></li>
                <li class="breadcrumb-item active"><?php echo $row['t_name']; ?></li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div id="content">
        <div class="container">
          <div class="row bar">
            ...
            <tr>
              <th>NAME</th>
              <td><input class="form-control" type="text"  name="t_name" value="<?php echo $row['t_name']; ?>"></td>
            </tr>

            <tr>
              <th>IC NUMBER</th>
              <td><input class="form-control" type="text" name="t_ic" value="<?php echo $row['t_ic']; ?>"></td>
            </tr>

            <tr>
              <th>ADDRESS</th>
              <td><input class="form-control" type="text"  name="t_address" value="<?php echo $row['t_address']; ?>"></td>
            </tr>
             
             <tr>
              <th>STATE</th>
              <td><select class="form-control" id="state_id" name="state_id">
        			<option value="<?php echo ($row1['state_id']); ?>" hidden><?php echo ($row1['state_name']); ?></option>
					<?php while ($row12=mysqli_fetch_array($result12,MYSQLI_ASSOC)){ ?>
					<option value="<?php echo $row12['state_id']; ?>" ><?php echo $row12['state_name']; ?></option>
					<?php } ?>
    			  </select></td>
             </tr>
              ...
              <tr>
              	<th>EMAIL</th>
              	<td><input class="form-control" type="text"  name="t_email" value="<?php echo $row['t_email']; ?>"></td>
              </tr>
              
              <tr>
              	<th>PHONE NUMBER</th>
              	<td><input class="form-control" type="text"  name="t_nophone" value="<?php echo $row['t_nophone']; ?>"></td>
              </tr>
              
              <tr>
              	<th>EDUCATION</th>
              	<td><select class="form-control" id="educationID" name="educationID">
      					<option value="<?php echo ($row2['educationID']); ?>" hidden><?php echo ($row2['education_level']); ?></option>	
						 <?php while ($row11=mysqli_fetch_array($result11,MYSQLI_ASSOC)) { ?>
						<option value="<?php echo ($row11['educationID']); ?>" ><?php echo $row11['education_level']; ?></option>
						 <?php }?>
    				</select></td>
              </tr>
              ...
              <tr>
              	...
              	<td><select class="form-control" id="g_id" name="g_id">
      				<option value="<?php echo ($row3['g_id']); ?>" hidden><?php echo ($row3['g_type']); ?></option>
					 <?php while ($row13=mysqli_fetch_array($result13,MYSQLI_ASSOC)) { ?>
					<option value="<?php echo ($row13['g_id']); ?>" ><?php echo $row13['g_type']; ?></option>
					 <?php }?>
    				</select></td>
              </tr>
              ...
              <tr>
              	 ...
              	 <td><select class="form-control" id="form_id" name="form_id">
      				<option value="<?php echo ($row5['form_id']); ?>" hidden><?php echo ($row5['form_name']); ?></option>
					 <?php while ($row15=mysqli_fetch_array($result15,MYSQLI_ASSOC)) { ?>
					<option value="<?php echo ($row15['form_id']); ?>" ><?php echo $row15['form_name']; ?></option>
					 <?php }?>
    				</select></td>
              </tr>
             ...
             <tr>
             	 <th>CLASS</th>
             	 <td><select class="form-control" id="class_id" name="class_id" value="<?php echo $row['class_id']; ?>">
						<option value="<?php echo ($row4['class_id']); ?>" hidden> <?php echo ($row4['class_name']); ?></option>
						 <?php while ($row14=mysqli_fetch_array($result14,MYSQLI_ASSOC)) { ?>
						<option value="<?php echo ($row14['class_id']); ?>" ><?php echo $row14['class_name']; ?></option>
						 <?php }?>
    				</select></td>
             </tr>
                    </tbody>
				  ...
				  <br><center><td><a href="info_teacher1.php?t_ic=<?php echo $t_ic ?>" class="btn btn-dark">CLOSE</a></td></center>
              </div>
            </div>
            ...
            <div class="col-lg-3 mt-4 mt-lg-0">
              ...
              <div class="panel panel-default sidebar-menu">
                ...
                <div class="panel-body">
                  <ul class="nav nav-pills flex-column text-sm">
                    <li class="nav-item"><a href="info_teacher1.php" class="nav-link active"><i class="fa fa-list"></i> Profile</a></li>
